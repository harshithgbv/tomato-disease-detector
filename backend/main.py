...
async def predict(file: UploadFile = File(...)):
    ...
    prompt = """You are an expert plant pathologist specializing in tomato diseases.
...
Rules:
- disease must be one of: Early Blight, Late Blight, Septoria Leaf Spot, Bacterial Spot, Leaf Mold, Spider Mites, Target Spot, Yellow Leaf Curl Virus, Mosaic Virus, Healthy
- confidence_score is a number 0-100
- tomato_score is how sure you are this is a tomato leaf (0-100)
- If not a tomato leaf, set status to "rejected" and tomato_score below 50"""
    ...
    if "choices" not in result:
        ...
        return {"status": "error", "disease": "API Error", "confidence": "Low", "treatment": error_msg}
    ...
    if match:
        ...
        return {"status": "error", "disease": "Unknown", "confidence": "Low", "treatment": "Could not analyze image."}
